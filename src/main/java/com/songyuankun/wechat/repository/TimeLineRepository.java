...
public interface TimeLineRepository extends JpaRepository<Article, Integer> {
    ...
    @Query(value = "select id,title,description,create_time  from article where DATE_FORMAT(create_time,'%Y')=#{year} and DATE_FORMAT(create_time,'%m')=1 and publish = 1", nativeQuery = true)
