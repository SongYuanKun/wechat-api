    ...
    private void genTimelineMonth(List<Timeline> timelineList) {
        for (Timeline timeline : timelineList) {
            ...
            for (int i = Calendar.DECEMBER + 1; i > 0; i--) {
                List<Object[]> objects = timeLineRepository.listTimelinePost(timeline.getYear(), 9);
                List<TimelinePost> postList = new ArrayList<>();
                objects.forEach(objects1 -> postList.add(new TimelinePost(Integer.parseInt(objects1[0].toString()), objects1[1].toString(), objects1[2].toString(), Date.valueOf(objects1[3].toString()))));
