...
public class DbUserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String uuid) {
        User byUuidKey = userRepository.findByUuidKey(uuid);
        if (byUuidKey == null) {
            throw new UsernameNotFoundException("用户不存在！");
        }
        List<SimpleGrantedAuthority> simpleGrantedAuthorities = new ArrayList<>();
        simpleGrantedAuthorities.add(new SimpleGrantedAuthority("USER"));
        return new org.springframework.security.core.userdetails.User(byUuidKey.getUserName(), "", simpleGrantedAuthorities);
    }
