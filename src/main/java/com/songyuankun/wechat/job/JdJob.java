...
public class JdJob {
    ...
    public String getJdSign() {
        ...
        post.addHeaders(header).body(body);
        HttpResponse execute = post.execute();
        log.info("京东每日签到{}", execute.getStatus() == HttpStatus.HTTP_OK ? "成功" : "失败");
        adminService.sendMessageToAdmin("京东每日签到" + (execute.getStatus() == HttpStatus.HTTP_OK ? "成功" : "失败"));
        return execute.body();
    }
    ...
    public String getSharkBean() {
        for (int i = 1; i < 8; i++) {
            ...
            if (success.equals("true")) {
                adminService.sendMessageToAdmin("京东摇京豆签到成功");
                return response;
            }
        }
        return null;
    ...
    public String getLottery() {
        ...
        String response = execute.body();
        log.info("response:{}", response);
        adminService.sendMessageToAdmin("京豆抽奖任务" + (execute.getStatus() == HttpStatus.HTTP_OK ? "成功" : "失败"));
        return response;
    ...
    public String plusSign() {
        ...
        HttpResponse execute = post.execute();
        String response = execute.body();
        adminService.sendMessageToAdmin("京东plus会员签到" + (execute.getStatus() == HttpStatus.HTTP_OK ? "成功" : "失败"));
        return response;
    }
}
